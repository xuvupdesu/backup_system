...
def backup_data(src_list, dest_base):
...
def cleanup_old_backups(dest_base, keep=5):
...
def detect_usb_by_label(target_label):
    """
    偵測是否有 USB 裝置符合指定卷標
    """
    ...
    for p in partitions:
        if 'removable' in p.opts or 'cdrom' in p.opts:
            try:
                vol_label = os.popen(f'vol {p.device}').read()
                if target_label in vol_label:
                    return p.mountpoint
            except Exception as e:
                continue
    return None

def wait_for_target_usb(label, timeout=60):
    """
    等待特定 USB 裝置插入，最多等待 timeout 秒
    """
    print(f"正在等待 USB 裝置（卷標：{label}）插入... 最多等待 {timeout} 秒")
    for _ in range(timeout):
        mount = detect_usb_by_label(label)
        if mount:
            print(f"偵測到裝置：{mount}")
            return mount
        time.sleep(1)
...
def main():
    ...
    if folders:
        ...
        if choice == "1":
            ...
            usb_label = input("請輸入目標 USB 裝置的標籤（如 KINGSTON）：").strip()
            mount_point = wait_for_target_usb(usb_label)
            if mount_point:
                usb_dest = os.path.join(mount_point, "AutoBackup")
                os.makedirs(usb_dest, exist_ok=True)
                backup_data(folders, usb_dest)
                cleanup_old_backups(usb_dest, BACKUP_CYCLE_DAYS)
